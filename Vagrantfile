...
Vagrant.configure("2") do |config|
	...
	config.vm.define "db" do |db|
		...
		db.vm.provision "shell", inline: "/bin/bash -c 'yum install -y -q htop glances psmisc'"
	...
	config.vm.define "vault1" do |vault|
		...
		vault.vm.provision "file", source: "cc.os.user-input/vault/4.vault_configure_dynamic.sh", destination: "#{sHOME}/4.vault_configure_dynamic.sh"
