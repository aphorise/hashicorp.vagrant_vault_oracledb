...
function vaultPluginSetup()
{
	...
	unzip -q -o vault-plugin-database-oracle_${PLUGIN}_linux_amd64.zip ;
	SHA256=$(sha256sum vault-plugin-database-oracle | cut -d' ' -f1) ;
	if ! vault write sys/plugins/catalog/database/oracle-database-plugin sha256="${SHA256}" command=vault-plugin-database-oracle 2>&1>/dev/null ; then
		printf "ERROR: Unable to regigster plugin.\n" ;
	fi ;
}
